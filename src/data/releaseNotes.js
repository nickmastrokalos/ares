...
export const RELEASES = [
  {
    ...
    changed: [
      ...
      'Map snapshot default filename changed from `<sanitized-mission-name>_<stamp>.png` to a fixed `ares_screen_capture_<stamp>.png`. Applies to both the toolbar button (still prompts via save dialog) and the new agent tool (writes directly to Desktop). The legend strip in the snapshot image is also re-titled — used to print the mission name (so e.g. a mission called "Test" became the bold legend title); now reads "Ares Map Snapshot" regardless of mission.',
