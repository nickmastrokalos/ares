...
export const RELEASES = [
  {
    ...
    fixed: [
      'AIS avoidance in route planning now stays clear of every vessel\'s current position regardless of approach angle. Previously a moving vessel was modelled only as a forward-swept rectangle along its COG, so a route approaching from abeam (perpendicular to the vessel\'s heading) could pass within the standoff distance of the vessel itself — the rectangle didn\'t cover that area. Each vessel now also contributes a circular keepout at its current position of radius `ais_standoff_meters`; the forward corridor stays for moving vessels but is no longer the only barrier.',
