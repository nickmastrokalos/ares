...
export const RELEASES = [
  {
    ...
    changed: [
      ...
      'AIS vessel breadcrumb trails now share the global `Track breadcrumbs` setting (Settings → Tracks) — same toggle and length slider as CoT tracks. The AIS panel\'s former "Heading tails" switch is renamed "Heading arrows" and now only governs whether vessel icons render as plain circles or direction-aware arrows. The yellow accents on AIS panel switches and status icons are gone; they use the app\'s primary theme color now. Assistant tool `ais_set_tails` is renamed `ais_set_heading_arrows`; legacy stored config under `aisBreadcrumbs` is read once on load and migrated.',
