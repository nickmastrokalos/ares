...
export const RELEASES = [
  {
    ...
    changed: [
      ...
      'ADS-B detail panel now shows altitude as a plain integer (e.g. `37,000 ft`) at every level instead of switching to flight-level shorthand above 18,000 ft.',
      'ADS-B aircraft visually float above the map when the camera is tilted past 25° — icons and labels offset upward in screen space proportional to altitude × sin(pitch − 25°). At low pitch the offset collapses to zero so markers stay anchored to ground positions; tilt the map and aircraft separate cleanly by altitude.'
    ],
    fixed: [
