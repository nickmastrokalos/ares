...
| `tools/waterRouting.js` | MapView | Land-aware route planning backed by OpenStreetMap simplified-land-polygons (`src/assets/osm-land-simplified.json`, ~58 MB, 67k polygons, lazy-loaded on first use, © OSM contributors / ODbL). `route_check_land_crossing` (read) returns the index of the first leg that enters land, or -1. `map_draw_route_water_only` (write) plans + draws a polyline from `start` to `end` that detours around land. Planner internals — grid A* with line-of-sight smoothing — live in `src/services/landRouting.js`. |
