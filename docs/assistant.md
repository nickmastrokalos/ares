...
| `tools/video.js` | MapView | `map_capture_video` (write) — records the MapLibre WebGL canvas via `canvas.captureStream` + `MediaRecorder` for `duration_seconds` (5 / 10 / 30 / 60), then writes the result to the user's Desktop as `ares_map_video_<UTC ISO timestamp>.<ext>`. Codec is selected at runtime (`MediaRecorder.isTypeSupported`) — typically WebM/VP9 on Chromium-backed webviews (WebView2), MP4/H.264 on WKWebView (macOS). Optional `filename` overrides the default. Note: only the WebGL canvas is captured — HTML overlay text (bullseye / bloodhound / perimeter labels) is NOT in the video; the snapshot tool composites those for stills. Backed by `useMapVideo.record({ destination: 'desktop' })`. |
