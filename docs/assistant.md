...
| `tools/waterRouting.js` | MapView | Routing tools backed by the grid-A* + bitmap planner in `src/services/landRouting.js`. `route_check_land_crossing` (read) tests an existing route's polyline against bundled Natural Earth 10m coastlines (`src/assets/ne-land-10m.json`, ~10 MB, lazy-loaded on first use). `map_draw_route_water_only` (write) plans + draws a polyline from `start` to `end` that detours around land — reliable for ocean-crossing and large-bay routes, generalizes at sub-km coastal scales so the tool description tells the LLM to warn the user. `map_draw_route_avoiding_features` (write) plans + draws a polyline that does NOT enter the bounding shape of supplied user-drawn polygon / box / circle / ellipse / sector features (keepouts, no-go zones); takes an optional `buffer_meters` standoff. Planner internals: scanline rasterization + dilation into a bitmap, A* on the bitmap, greedy line-of-sight smoothing against that same bitmap (no length cap — the buffered LOS test is the safety mechanism). Coastline buffer (`LAND_BUFFER_DEG`, ~555 m) absorbs NE 10m's generalization error for water routing only — feature-avoidance defaults to no buffer. |
