...
| `tools/waterRouting.js` | MapView | Routing tools backed by the grid-A* + bitmap planner in `src/services/landRouting.js`. `route_check_land_crossing` (read) tests an existing route's polyline against bundled Natural Earth 10m coastlines (`src/assets/ne-land-10m.json`, ~10 MB, lazy-loaded on first use). `map_draw_route_water_only` (write) plans + draws a polyline that detours around land — reliable for ocean-crossing and large-bay routes, generalizes at sub-km coastal scales so the tool description tells the LLM to warn the user. `map_draw_route_avoiding_features` (write) is the multi-constraint route tool: stacks any combination of `avoid_feature_ids` (route does not enter these shapes), `via_feature_ids` (route passes through these shapes' centers in order), `avoid_land: true` (also stays off the bundled coastline), and `avoid_ais: true` (every AIS vessel gets a circular keepout of radius `ais_standoff_meters` at its current position; moving vessels additionally get a swept corridor of the same half-width extending `ais_horizon_minutes` forward along COG). Each via shape contributes one intermediate waypoint at its center; legs are planned independently via `planRouteAvoidingObstacles` and concatenated. Planner internals: scanline rasterization + dilation into per-layer bitmaps, OR'd together when multiple obstacle types are stacked. A* on the combined bitmap, greedy line-of-sight smoothing against that bitmap (no length cap — the buffered LOS test is the safety mechanism). |
| `tools/ais.js`    | MapView | Config: `ais_get_status`, `ais_set_enabled` (refuses when feed URL / API key unset), `ais_set_visible`, `ais_set_heading_arrows` (icon style — circle vs heading-rotated arrow). Vessel queries: `ais_list_vessels` (optional name substring filter), `ais_vessels_near` (radius search around a feature or coordinate, sorted nearest-first). The history-trail rendering for AIS shares the global `Track breadcrumbs` setting, so there's no per-AIS toggle for it. |
| `tools/adsb.js`   | MapView | Free key-less aircraft feed from airplanes.live. Config: `adsb_get_status`, `adsb_set_enabled` (no key gate — endpoint is unauthenticated), `adsb_set_visible`, `adsb_set_heading_arrows` (icon style — circle vs track-rotated arrow). Aircraft queries: `adsb_list_aircraft` (optional callsign-or-hex substring filter), `adsb_aircraft_near` (radius search around a feature or coordinate, sorted nearest-first). Returned shape includes `hex`, `callsign`, `registration`, `type`, `coordinate`, `altitudeFt` (0 for on-ground), `speedKnots`, `trackDeg`, `headingDeg`, `squawk`. Rate-limited to 1 req/sec server-side; the composable polls every 10 s. |
...
| `tools/video.js` | MapView | `map_capture_video` (write) — records the MapLibre WebGL canvas via `canvas.captureStream` + `MediaRecorder` for `duration_seconds` (any integer 1–60), then writes the result to the user's Desktop as `ares_map_video_<UTC ISO timestamp>.<ext>`. Codec is selected at runtime (`MediaRecorder.isTypeSupported`) — typically WebM/VP9 on Chromium-backed webviews (WebView2), MP4/H.264 on WKWebView (macOS). Optional `filename` overrides the default. The toolbar's video button uses fixed durations (5/10/30/60); the agent path is free-form. Note: only the WebGL canvas is captured — HTML overlay text (bullseye / bloodhound / perimeter labels) is NOT in the video; the snapshot tool composites those for stills. Backed by `useMapVideo.record({ destination: 'desktop' })`. |
