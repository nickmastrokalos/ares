...
| `tools/waterRouting.js` | MapView | Land-aware route planning backed by Natural Earth 10m coastlines (`src/assets/ne-land-10m.json`, ~10 MB, lazy-loaded on first use). `route_check_land_crossing` (read) returns the index of the first leg that enters land, or -1. `map_draw_route_water_only` (write) plans + draws a polyline from `start` to `end` that detours around land — reliable for ocean-crossing and large-bay routes, generalizes too aggressively at sub-km coastal scales (small bays, barrier islands) so the tool description tells the LLM to warn the user when the route is short. Planner internals — grid A* with smoothing capped at 1.5 km per merged leg — live in `src/services/landRouting.js`. |
